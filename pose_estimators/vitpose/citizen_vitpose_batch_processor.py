...
# ============================================================
# Input/output paths
INPUT_VIDEO_PATH = "/shares/iict-sp2.ebling.cl.uzh/common/ASL_Citizen/videos/"
OUTPUT_JSON_PATH = "/shares/iict-sp2.ebling.cl.uzh/common/ASL_Citizen/ViTPose/"
...
def main():
    ...
    # Start prefetch
    print(f"\n🚀 Starting processing with prefetch (queue={PREFETCH_QUEUE_SIZE})...")
    prefetcher = PrefetchVideoProcessor(video_list)
    prefetcher.start()
    ...
    while True:
        ...
        if 'error' in video_info or not video_info.get('frames'):
            failed += 1
            if pbar:
                pbar.update(1)
            else:
        ...
        try:
            ...
            if success:
                completed += 1
                fps = len(video_info['frames']) / max(time.time() - t0, 0.01)
                if pbar:
                    pbar.set_postfix({"fps": f"{fps:.1f}", "✅": completed, "❌": failed})
                else:
                    print(f"  ✅ {os.path.basename(video_path)} ({fps:.1f} fps)")
            else:
                failed += 1
